...
#torchanncmd = "TORCHANNCMD"
torchanncmd_f = open("torchanncmd.txt", "rt")
torchanncmd = torchanncmd_f.readlines()[0]
torchanncmd_f.close()
...
def torchannrun_read():
  ...
  stensor = np.loadtxt("S_tmp")
  f_virial_all = open("virial_all.raw", "a")
  #print(virial_np.reshape(-1,))
  np.savetxt(f_virial_all, stensor.reshape(1,-1), fmt="%10.6f")
  f_virial_all.close()
  stensor = stensor.tolist()
  sxx = stensor[0][0]
  syy = stensor[1][1]
  szz = stensor[2][2]
  # symmetrize off-diagonal components
