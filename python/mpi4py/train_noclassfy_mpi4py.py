...
if (hvd.rank() == 0):
    DATA_ON_RANK_0 = read_and_init_bin_file(parameters, default_dtype=default_dtype)
    ...
    N_ATOMS_ORI_tf = DATA_ON_RANK_0
    SYM_COORD_Reshape_tf_for_other_ranks = tf.zeros(len(SYM_COORD_Reshape_tf))
    SYM_COORD_DX_Reshape_tf_for_other_ranks = tf.zeros(len(SYM_COORD_DX_Reshape_tf))
    SYM_COORD_DY_Reshape_tf_for_other_ranks = tf.zeros(len(SYM_COORD_DY_Reshape_tf))
    SYM_COORD_DZ_Reshape_tf_for_other_ranks = tf.zeros(len(SYM_COORD_DZ_Reshape_tf))
    DATA_FOR_OTHER_RANKS = (
    COORD_Reshape_tf, SYM_COORD_Reshape_tf_for_other_ranks, ENERGY_tf, FORCE_Reshape_tf, N_ATOMS_tf, TYPE_Reshape_tf,
    NEI_IDX_Reshape_tf, NEI_COORD_Reshape_tf, FRAME_IDX_tf, SYM_COORD_DX_Reshape_tf_for_other_ranks,
    SYM_COORD_DY_Reshape_tf_for_other_ranks, SYM_COORD_DZ_Reshape_tf_for_other_ranks, N_ATOMS_ORI_tf)
else:
    DATA_ON_RANK_0 = None
    DATA_FOR_OTHER_RANKS = None
DATA_FOR_OTHER_RANKS = MPI.COMM_WORLD.bcast(DATA_FOR_OTHER_RANKS, root=0)
if (hvd.rank() != 0):
    COORD_Reshape_tf, SYM_COORD_Reshape_tf, ENERGY_tf, FORCE_Reshape_tf, N_ATOMS_tf, TYPE_Reshape_tf, NEI_IDX_Reshape_tf, \
    NEI_COORD_Reshape_tf, FRAME_IDX_tf, SYM_COORD_DX_Reshape_tf, SYM_COORD_DY_Reshape_tf, SYM_COORD_DZ_Reshape_tf, \
    N_ATOMS_ORI_tf= DATA_FOR_OTHER_RANKS
...
#with tf.autograd.profiler.profile(enabled = True, use_cuda=True) as prof:
    ...
    for epoch in range(parameters.epoch):
        ...
        for batch_idx, data_cur in enumerate(TRAIN_LOADER):
            ...
            ###First, gather all the FRAME_IDX from all processes
            ###Second, use index_select to get SYM_COORD, DX, DY, DZ for every process
            ###Finally, scatter.
            FRAME_IDX_from_all_proc = MPI.COMM_WORLD.gather(data_cur[8], root=0)
            if (hvd.rank() == 0):
                SYM_COORD_all_proc = []
                SYM_COORD_DX_all_proc = []
                SYM_COORD_DY_all_proc = []
                SYM_COORD_DZ_all_proc = []
                for proc_idx in range(hvd.size()):
                    SYM_COORD_all_proc.append(
                        tf.index_select(SYM_COORD_Reshape_tf, 0, FRAME_IDX_from_all_proc[proc_idx].long()))
                    SYM_COORD_DX_all_proc.append(
                        tf.index_select(SYM_COORD_DX_Reshape_tf, 0, FRAME_IDX_from_all_proc[proc_idx].long()))
                    SYM_COORD_DY_all_proc.append(
                        tf.index_select(SYM_COORD_DX_Reshape_tf, 0, FRAME_IDX_from_all_proc[proc_idx].long()))
                    SYM_COORD_DZ_all_proc.append(
                        tf.index_select(SYM_COORD_DX_Reshape_tf, 0, FRAME_IDX_from_all_proc[proc_idx].long()))
            else:
                SYM_COORD_all_proc = None
                SYM_COORD_DX_all_proc = None
                SYM_COORD_DY_all_proc = None
                SYM_COORD_DZ_all_proc = None
            data_cur[1] = MPI.COMM_WORLD.scatter(SYM_COORD_all_proc, root = 0)
            data_cur[9] = MPI.COMM_WORLD.scatter(SYM_COORD_DX_all_proc, root = 0)
            data_cur[10] = MPI.COMM_WORLD.scatter(SYM_COORD_DY_all_proc, root=0)
            data_cur[11] = MPI.COMM_WORLD.scatter(SYM_COORD_DZ_all_proc, root=0)
            ...
            with tf.autograd.profiler.profile(enabled=PROF_FLAG, use_cuda=True) as prof:
                ...
                # broadcast std, avg, use_std_avg to every process
                if (STEP_CUR == 0):
                    if (hvd.rank() == 0):
                        additional_param = (std, avg, use_std_avg)
                    else:
                        additional_param = None
                    additional_param = MPI.COMM_WORLD.bcast(additional_param, root = 0)
                    std, avg, use_std_avg = additional_param
                ...
                if (STEP_CUR % (500 // hvd.size()) == 0):
                    print("Force check:\n", F_cur_batch[0].data)
                    print("additional_param check:\n", additional_param)
                    if (hvd.rank() == DEBUG_RANK):
                        f_out = open("./LOSS.OUT", "a")
                        print("Force check:\n", F_cur_batch.data[0], file=f_out)
                        print("additional_param check:\n", additional_param, file=f_out)
                        f_out.close()
                ...
                """if (STEP_CUR >= 2):
                    break"""
            if (PROF_FLAG and hvd.rank() == DEBUG_RANK):
