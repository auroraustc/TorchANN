            ...
            with tf.autograd.profiler.profile(enabled=PROF_FLAG, use_cuda=True) as prof:
                ...
                if ((batch_idx  == 0 and epoch % (parameters.output_epoch) == 0) or ((epoch == parameters.stop_epoch - 1) and (batch_idx == 0))):
                    ...
                    print("Epoch: %-10d, Batch: %-10d, lossE: %10.6f eV/atom, lossF: %10.6f eV/A, time: %10.3f s" % (
                        epoch, batch_idx, tf.mean(tf.sqrt(loss_E_cur_batch) / data_cur[12].double()),
                        tf.sqrt(tf.sum(loss_F_cur_batch) / 3.0 / tf.sum(data_cur[12].double())),
                    ...
                    if (True):
                        ...
                        print(
                            "Epoch: %-10d, Batch: %-10d, lossE: %10.6f eV/atom, lossF: %10.6f eV/A, time: %10.3f s" % (
                                epoch, batch_idx,
                                tf.mean(tf.sqrt(loss_E_cur_batch) / data_cur[12].double()),
                                tf.sqrt(tf.sum(loss_F_cur_batch) / 3.0 / tf.sum(data_cur[12].double())),
