...
def read_and_init_bin_file_all(parameters, default_dtype):
    COORD = np.fromfile("./COORD.BIN", dtype=np.float32)
    SYM_COORD = np.fromfile("./SYM_COORD.BIN", dtype=np.float32)
    ENERGY = np.fromfile("./ENERGY.BIN", dtype=np.float32)
    FORCE = np.fromfile("./FORCE.BIN", dtype=np.float32)
    ...
    NEI_COORD = np.fromfile("./NEI_COORD.BIN", dtype=np.float32)
    SYM_COORD_DX = np.fromfile("./SYM_COORD_DX.BIN", dtype=np.float32)
    SYM_COORD_DY = np.fromfile("./SYM_COORD_DY.BIN", dtype=np.float32)
    SYM_COORD_DZ = np.fromfile("./SYM_COORD_DZ.BIN", dtype=np.float32)
...
def read_and_init_bin_file(parameters, default_dtype, is_predict=0):
    ...
    if (is_predict == 0):
        ...
        ENERGY = np.zeros(shape=(parameters.Nframes_tot, ), dtype= np.float32)
        FORCE = np.zeros(shape=COORD.shape, dtype= np.float32)
    TYPE = np.fromfile("./TYPE.BIN", dtype=np.int32)
    N_ATOMS = np.fromfile("./N_ATOMS.BIN", dtype=np.int32)
    NEI_IDX = np.fromfile("./NEI_IDX.BIN", dtype=np.int32)
    NEI_COORD = np.fromfile("./NEI_COORD.BIN", dtype= np.float32)
    ...
    TYPE_Reshape_tf = tf.from_numpy(TYPE_Reshape)
    NEI_IDX_Reshape_tf = tf.from_numpy(NEI_IDX_Reshape).long()
    NEI_COORD_Reshape_tf = tf.from_numpy(NEI_COORD_Reshape).type(tf.float32)
    #NEI_COORD_Reshape_tf = tf.zeros(len(SYM_COORD_Reshape_tf))
    FRAME_IDX_tf = tf.ones(len(COORD_Reshape_tf), dtype=tf.int32)
    for i in range(len(FRAME_IDX_tf)):
        FRAME_IDX_tf[i] = i
