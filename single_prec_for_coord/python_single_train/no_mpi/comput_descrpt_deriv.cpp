...
std::vector<torch::Tensor> calc_descrpt_and_deriv_DPMD(torch::Tensor COORD, torch::Tensor NEI_COORD, torch::Tensor NEI_DIST, int Nframes_tot, int N_Atoms_max, int SEL_A_max, float rcs, float rc)
{
    ...
    auto COORD_RESHAPE_ACCESSOR = COORD_RESHAPE.accessor<float, 3>();
    auto NEI_COORD_RESHAPE_ACCESSOR = NEI_COORD_RESHAPE.accessor<float, 4>();
    auto SYM_COORD_DPMD_RESHAPE_ACCESSOR = SYM_COORD_DPMD_RESHAPE.accessor<float, 3>();
    auto SYM_COORD_DPMD_DX_RESHAPE_ACCESSOR = SYM_COORD_DPMD_DX_RESHAPE.accessor<float, 3>();
    auto SYM_COORD_DPMD_DY_RESHAPE_ACCESSOR = SYM_COORD_DPMD_DY_RESHAPE.accessor<float, 3>();
    auto SYM_COORD_DPMD_DZ_RESHAPE_ACCESSOR = SYM_COORD_DPMD_DZ_RESHAPE.accessor<float, 3>();
