...
__device__ float s_r(float r_ij, float r_c, float r_cs)
{
    float result;
    float rc = r_c;
    float rcs = r_cs;
...
__device__ float fastpow2(float number, int dummy)
{
    return number * number;
}

__global__ void calc_descrpt_and_deriv_DPMD_kernel(int Nframes_tot, int N_Atoms_max, int SEL_A_max, float rc, float rcs, torch::PackedTensorAccessor<float, 3> COORD_RESHAPE_ACCESSOR, torch::PackedTensorAccessor<float, 4> NEI_COORD_RESHAPE_ACCESSOR, torch::PackedTensorAccessor<float, 3> SYM_COORD_DPMD_RESHAPE_ACCESSOR, torch::PackedTensorAccessor<float, 3> SYM_COORD_DPMD_DX_RESHAPE_ACCESSOR, torch::PackedTensorAccessor<float, 3> SYM_COORD_DPMD_DY_RESHAPE_ACCESSOR, torch::PackedTensorAccessor<float, 3> SYM_COORD_DPMD_DZ_RESHAPE_ACCESSOR)
{
    ...
    for (k = 0; k <= SEL_A_max - 1; k++)
    {
        float four_coord[4];
        float r_ij;
        float atom_coord[3] = {COORD_RESHAPE_ACCESSOR[i][j][0], COORD_RESHAPE_ACCESSOR[i][j][1], COORD_RESHAPE_ACCESSOR[i][j][2]};
        float nei_coord[3] = {NEI_COORD_RESHAPE_ACCESSOR[i][j][k][0], NEI_COORD_RESHAPE_ACCESSOR[i][j][k][1], NEI_COORD_RESHAPE_ACCESSOR[i][j][k][2]};
        float r_ji_coord[3] = {nei_coord[0] - atom_coord[0], nei_coord[1] - atom_coord[1], nei_coord[2] - atom_coord[2]};
...
std::vector<torch::Tensor> calc_descrpt_and_deriv_DPMD(torch::Tensor COORD, torch::Tensor NEI_COORD, torch::Tensor NEI_DIST, int Nframes_tot, int N_Atoms_max, int SEL_A_max, float rcs, float rc)
{

    torch::Tensor SYM_COORD_DPMD_RESHAPE = torch::zeros({Nframes_tot, N_Atoms_max, SEL_A_max * 4}, torch::TensorOptions().dtype(torch::kFloat32).device(torch::kCUDA));
    torch::Tensor SYM_COORD_DPMD_DX_RESHAPE = torch::zeros({Nframes_tot, N_Atoms_max, SEL_A_max * 4}, torch::TensorOptions().dtype(torch::kFloat32).device(torch::kCUDA));
    torch::Tensor SYM_COORD_DPMD_DY_RESHAPE = torch::zeros({Nframes_tot, N_Atoms_max, SEL_A_max * 4}, torch::TensorOptions().dtype(torch::kFloat32).device(torch::kCUDA));
    torch::Tensor SYM_COORD_DPMD_DZ_RESHAPE = torch::zeros({Nframes_tot, N_Atoms_max, SEL_A_max * 4}, torch::TensorOptions().dtype(torch::kFloat32).device(torch::kCUDA));
    ...
    auto COORD_RESHAPE_ACCESSOR = COORD_RESHAPE.packed_accessor<float, 3>();
    auto NEI_COORD_RESHAPE_ACCESSOR = NEI_COORD_RESHAPE.packed_accessor<float, 4>();
    auto SYM_COORD_DPMD_RESHAPE_ACCESSOR = SYM_COORD_DPMD_RESHAPE.packed_accessor<float, 3>();
    auto SYM_COORD_DPMD_DX_RESHAPE_ACCESSOR = SYM_COORD_DPMD_DX_RESHAPE.packed_accessor<float, 3>();
    auto SYM_COORD_DPMD_DY_RESHAPE_ACCESSOR = SYM_COORD_DPMD_DY_RESHAPE.packed_accessor<float, 3>();
    auto SYM_COORD_DPMD_DZ_RESHAPE_ACCESSOR = SYM_COORD_DPMD_DZ_RESHAPE.packed_accessor<float, 3>();
