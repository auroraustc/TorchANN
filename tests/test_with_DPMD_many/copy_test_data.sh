...
test_num=$(rand 5 $tot_num_dataset)
...
cd ../
cd TorchANN
sed -n -e '/Force/,/Additional/p' train_runlog | sed '/Force/d' | sed '/Add/d' | sed '/dev/d' | sed 's/\[/\ /g' | sed 's/\]/\ /g' | sed 's/,/\ /g' | sed 's/tensor(//g' > force_TorchANN
cd ../
